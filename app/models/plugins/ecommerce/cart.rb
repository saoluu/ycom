class Plugins::Ecommerce::Cart < ActiveRecord::Base
  ...
  def add_product(product, qty = 1, variation_id = nil, booking_date=nil)
    pi = product_items.where(product_id: product.id, variation_id: variation_id).first

    if pi.present?
      pi.update_column(:qty, qty)
    else
      pi = product_items.create(product_id: product.id, qty: qty, variation_id: variation_id, booking_date: booking_date)
    end
    pi
  end
