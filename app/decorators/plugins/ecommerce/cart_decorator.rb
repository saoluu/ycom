class Plugins::Ecommerce::CartDecorator < Draper::Decorator
  ...
  def booking_time
   @booking_dateee = Time.at(1364046539).strftime("%B %e, %Y at %I:%M %p") # 'cake' or 'pie'
  end
    def booking_timesss (booking_dateee = Time.at(1364046539).strftime("%B %e, %Y at %I:%M %p"))
   @booking_dateee = params[:booking_dateee].to_s
