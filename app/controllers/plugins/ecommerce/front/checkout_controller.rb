class Plugins::Ecommerce::Front::CheckoutController < Plugins::Ecommerce::FrontController
  ...
  def index
    unless @cart.product_items.count > 0
      flash[:cama_ecommerce][:notice] = t('plugins.ecommerce.messages.cart_no_products', default: 'Not exist services in your cart')
  ...
  def step_shipping
    @cart.product_items.first.update_column(:qty, params[:product_qty])
  ...
  def cart_index
    ...
    @ecommerce_breadcrumb << [t('plugins.ecommerce.messages.shopping_cart', default: 'Booking cart')]
  end
  ...
  def cart_add
    ...
    unless product.valid_variation?(params[:variation_id])
      flash[:cama_ecommerce][:error] = t('plugins.ecommerce.messages.missing_variation', default: 'Invalid Service Variation')
    ...
    unless product.can_added?(qty, params[:variation_id])
      flash[:cama_ecommerce][:error] =  t('plugins.ecommerce.messages.not_enough_product_qty', product: product.the_variation_title(params[:variation_id]), qty: product.the_qty(params[:variation_id]), default: 'There is not enough services "%{product}" (Available %{qty})')
      return params[:format] == 'json' ? render(json: flash.discard(:cama_ecommerce).to_hash) : (redirect_to :back)
    end
    @cart.add_product(product, qty, params[:variation_id], booking_date)
    flash[:cama_ecommerce][:notice] = t('plugins.ecommerce.messages.added_product_in_cart', default: 'Service added into cart')
    params[:format] == 'json' ? render(json: flash.discard(:cama_ecommerce).to_hash) : (redirect_to plugins_ecommerce_checkout_path)
  end
  ...
  def cart_update
    ...
    params[:product_items].each do |data|
      ...
      if product.can_added?(qty, item.variation_id)
        ...
        errors << t('plugins.ecommerce.messages.not_enough_product_qty', product: product.the_variation_title(item.variation_id), qty: product.the_qty(item.variation_id), default: 'There is not enough services "%{product}" (Available %{qty})')
      end
    end
    flash[:cama_ecommerce][:error] = errors.join('<br>') if errors.present?
    flash[:cama_ecommerce][:notice] = t('plugins.ecommerce.messages.cart_updated', default: 'Booking cart updated') unless errors.present?
  ...
  def cart_remove
    ...
    flash[:cama_ecommerce][:notice] = t('plugins.ecommerce.messages.cart_deleted', default: 'Service removed from your Booking cart')
    params[:format] == 'json' ? render(json: flash.discard(:cama_ecommerce).to_hash) : (redirect_to action: :cart_index)
  end
