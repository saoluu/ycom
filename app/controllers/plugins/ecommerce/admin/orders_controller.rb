class Plugins::Ecommerce::Admin::OrdersController < Plugins::Ecommerce::AdminController
  ...
  def show
    ...
    add_breadcrumb("#{t('plugins.ecommerce.details_order', default: 'Booking details')} - #{@order.slug}")
  end
  ...
  def update
    ...
    flash[:notice] = "#{t('plugins.ecommerce.message.order_updated', default: 'Booking Updated')}"
  ...
  def destroy
    ...
    if @order.destroy
      flash[:notice] = "#{t('plugins.ecommerce.message.order_destroyed', default: 'Booking Destroyed')}"
    else
      flash[:error] = "#{t('plugins.ecommerce.message.order_no_destroyed', default: 'Occurred some problems destroying the Booking')}"
    end
    hooks_run('plugin_ecommerce_after_destroy_order', @order)
  ...
  def mark_accepted
    ...
    message = "#{t('plugins.ecommerce.message.order_accepted', default: 'Booking Accepted')}"
  ...
  def mark_shipped
    ...
    flash[:notice] = "#{t('plugins.ecommerce.message.order_shipped', default: 'Booking Checked')}"
  ...
  def mark_canceled
    ...
    flash[:notice] = "#{t('plugins.ecommerce.message.order_canceled', default: 'Booking canceled')}"
  ...
  def resend_email
    ...
    flash[:notice] = "#{t('plugins.ecommerce.message.order_email_resent', default: 'Booking Email Resent')}"
    redirect_to action: :index
  end
