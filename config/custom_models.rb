    ...
    def e_email_for(key = 'email_order_paid')
      (e_settings[key]) || case key.to_s
                                  when 'email_order_received'
                                    '<h1>ORDER SUMMARY {number}</h1> Dear {name}, please review and retain the following booking information for your records.<br>{order_table}'
                                  when 'email_order_confirmed'
                                    '<h1>ORDER CONFIRMED {number}</h1> Dear {name}, your booking has been confirmed. Please retain the following booking information for your booking<br>{order_table}'
                                  when 'email_order_shipped'
                                    '<h1>SHIPMENT SUMMARY</h1> Dear {name}, your booking {number} has been shipped. Shipped method: {shipping_name} <br>{order_table}<br>{tracking_url}'
                                  when 'email_order_cancelled'
                                    '<h1>ORDER {number} CANCELLED</h1> Dear {name}, your booking has been cancelled. Please retain this cancellation information for your booking. <br>{order_table}'
                                  when 'email_order_invoice'
                                    '<table style="width: 100%;"><tr><td><h1>INVOICE #{invoice_number}</h1> <h4>Order #{number}</h4><div>{current_date}</div></td><td style="text-align: center;"><img src=""></td></tr></table> <table style="width: 100%;"><tr><td><strong>Billing Address</strong><br>{billing_info}</td><td><strong>Resident Address</strong><br>{shipping_info}</td></tr><tr><td colspan="2">{order_table}</td></tr></table>'
                                end
    end
